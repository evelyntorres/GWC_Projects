...
class Scroller(object):
	...
	def add_buildings(self):
		currentwidth = 0
		while currentwidth <= 600:
			self.add_building(currentwidth)
			currentwidth=currentwidth + self.buildings_list [-1].width
			self.scroll_end = currentwidth
	...
	def add_building(self, x_location):
		locationalwidth = random.randint(0,200)
		locationalheight = random.randint(0,300)
		heightmax = self.base - self.height 
		y_location = self.base - locationalheight
		self.buildings_list.append(Building(x_location, y_location, locationalwidth, locationalheight, self.color,))
	...
	def move_buildings(self):
		for building in self.buildings_list:
			building.move(random.randint(-10,0))
			
		newbuilding = self.buildings_list [-1].x_point + self.buildings_list [-1].width
		self.add_building(newbuilding)
